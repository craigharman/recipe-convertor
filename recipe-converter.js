...
class RecipeConverter {
    ...
    extractIngredients(structuredData, document) {
        ...
        if (structuredData && structuredData.recipeIngredient) {
            ...
            const ingredientElements = document.querySelectorAll('[itemprop="recipeIngredient"]');
            ingredients = Array.from(ingredientElements).map(el => el.textContent.trim());
        }
    ...
    extractInstructions(structuredData, document) {
        ...
        if (structuredData && structuredData.recipeInstructions) {
            ...
            const instructionElements = document.querySelectorAll('[itemprop="recipeInstruction"]');
    ...
    extractSource(structuredData, document) {
        if (structuredData && structuredData.author) {
            if (typeof structuredData.author === 'object' && structuredData.author.url) {
            ...
            protocol.get(url, (response) => {
                ...
                response.on('end', () => {
                    try {
                        ...
                        console.warn(`Error converting image to base64: ${url}`, error.message);
                        resolve(null);
                    }
                });
            }).on('error', (error) => {
    ...
    async convertYMLFiles() {
        ...
        const ymlFiles = fs.readdirSync(ymlDir).filter(file => file.endsWith('.yml'));
        const convertedRecipes = [];
    ...
    createMelaRecipesFile(recipes) {
        ...
        const output = fs.createWriteStream(path.join(this.outputDir, 'recipes.melarecipes'));
        ...
        recipes.forEach(recipe => {
            archive.append(JSON.stringify(recipe, null, 2), { name: `${recipe.id}.melarecipe` });
        });
